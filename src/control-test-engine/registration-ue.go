...
func registrationUE(connN2 *sctp.SCTPConn, ueId int, ranUeId int64, ranIpAddr string) error {
	var sendMsg []byte
	...
	if err != nil {
		return fmt.Errorf("Error getting %s ue initial message", ue.Supi)
	}
	...
	if err != nil {
		return fmt.Errorf("Error getting %s NAS Authentication Response", ue.Supi)
	}

	_, err = connN2.Write(sendMsg)
	if err != nil {
		return fmt.Errorf("Error sending %s NAS Authentication Response", ue.Supi)
	}

	// receive NAS Security Mode Command Msg
	n, err = connN2.Read(recvMsg)
	if err != nil {
		return fmt.Errorf("Error reading %s NAS Security Mode Command Message", ue.Supi)
	}
	_, err = ngap.Decoder(recvMsg[:n])
	if err != nil {
		return fmt.Errorf("Error decoding %s NAS Security Mode Command Message", ue.Supi)
	}
	...
	if err != nil {
		return fmt.Errorf("Error encoding %s ue NAS Security Mode Complete Message", ue.Supi)
	}
	sendMsg, err = ngap_control.GetUplinkNASTransport(ue.AmfUeNgapId, ue.RanUeNgapId, pdu)
	if err != nil {
		return fmt.Errorf("Error getting %s ue NAS Security Mode Complete Message", ue.Supi)
	}
	_, err = connN2.Write(sendMsg)
	if err != nil {
		return fmt.Errorf("Error sending %s ue NAS Security Mode Complete Message", ue.Supi)
	}

	// receive ngap Initial Context Setup Request Msg
	n, err = connN2.Read(recvMsg)
	if err != nil {
		return fmt.Errorf("Error receiving %s ue ngap Initial Context Setup Request Msg", ue.Supi)
	}
	_, err = ngap.Decoder(recvMsg[:n])
	if err != nil {
		return fmt.Errorf("Error decoding %s ue ngap Initial Context Setup Request Msg", ue.Supi)
	}

	// send ngap Initial Context Setup Response Msg
	sendMsg, err = ngap_control.GetInitialContextSetupResponse(ue.AmfUeNgapId, ue.RanUeNgapId)
	if err != nil {
		return fmt.Errorf("Error getting %s ue ngap Initial Context Setup Response Msg", ue.Supi)
	}
	_, err = connN2.Write(sendMsg)
	if err != nil {
		return fmt.Errorf("Error sending %s ue ngap Initial Context Setup Response Msg", ue.Supi)
	}
	...
	if err != nil {
		return fmt.Errorf("Error encoding %s ue NAS Registration Complete Msg", ue.Supi)
	}
	sendMsg, err = ngap_control.GetUplinkNASTransport(ue.AmfUeNgapId, ue.RanUeNgapId, pdu)
	if err != nil {
		return fmt.Errorf("Error getting %s ue NAS Registration Complete Msg", ue.Supi)
	}
	_, err = connN2.Write(sendMsg)
	if err != nil {
		return fmt.Errorf("Error sending %s ue NAS Registration Complete Msg", ue.Supi)
	}
	...
	pdu, err = nas_control.EncodeNasPduWithSecurity(ue, pdu, nas.SecurityHeaderTypeIntegrityProtectedAndCiphered, true, false)
	if err != nil {
		return fmt.Errorf("Error encoding %s ue PduSession Establishment Request Msg", ue.Supi)
	}

	sendMsg, err = ngap_control.GetUplinkNASTransport(ue.AmfUeNgapId, ue.RanUeNgapId, pdu)
	if err != nil {
		return fmt.Errorf("Error getting %s ue PduSession Establishment Request Msg", ue.Supi)
	}

	_, err = connN2.Write(sendMsg)
	if err != nil {
		return fmt.Errorf("Error sending %s ue PduSession Establishment Request Msg", ue.Supi)
	}

	// receive 12. NGAP-PDU Session Resource Setup Request(DL nas transport((NAS msg-PDU session setup Accept)))
	n, err = connN2.Read(recvMsg)
	if err != nil {
		return fmt.Errorf("Error reading %s ue NGAP-PDU Session Establishment Setup accept", ue.Supi)
	}
	_, err = ngap.Decoder(recvMsg[:n])
	if err != nil {
		return fmt.Errorf("Error decoding %s ue NGAP-PDU Session Establishment Setup accept", ue.Supi)
	}

	// send 14. NGAP-PDU Session Resource Setup Response.
	sendMsg, err = ngap_control.GetPDUSessionResourceSetupResponse(ue.AmfUeNgapId, ue.RanUeNgapId, ranIpAddr)
	if err != nil {
		return fmt.Errorf("Error getting %s ue NGAP-PDU Session Resource Setup Response", ue.Supi)
	}
	_, err = connN2.Write(sendMsg)
	if err != nil {
		return fmt.Errorf("Error sending %s ue NGAP-PDU Session Resource Setup Response", ue.Supi)
	}
