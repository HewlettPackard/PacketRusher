...
func gnbInitMock(gnbConf config.GNodeB, amfConf config.AMF, wg *sync.WaitGroup) *context.GNBContext {
	// instance new gnb.
	...
	gnb.NewGnBAmf(amfConf.Ip, amfConf.Port)
...
func TestSetupGnbs(t *testing.T) {
	...
	for _, scenario := range scenarios {
		t.Run(scenario.description, func(t *testing.T) {
			s := scenario.input.manager
			s.setupGnbs(scenario.input.gnbConfs, scenario.input.amfConf)
...
func TestSetupUeTaskExecuter(t *testing.T) {
	...
	gnbs := gnbInitMock(gnbConf, amfConf, wg)
	tasks := []Task{{
		TaskType: Registration,
		Delay:    2000,
	}, {
	...
	for _, scenario := range scenarios {
		t.Run(scenario.description, func(t *testing.T) {
			...
			time.Sleep(time.Duration(scenario.input.ueScenario.ForceStop) * time.Millisecond)

			if scenario.expected.forceStop {
				select {
				case <-s.reportChan:
				default:
					t.Error("did not receive report for terminating ue")
				}
			}
			assert.Equal(t, scenario.expected.forceStop, s.ues[scenario.input.ueId].done)
		})
	}
...
func TestSetupUeTaskExecuterHasError(t *testing.T) {
	...
	gnb := gnbInitMock(gnbConf, amfConf, wg)
	tasks := []Task{{
		TaskType: Registration,
		Delay:    2000,
	}, {
