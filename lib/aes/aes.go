...
func GETU32(key []uint8, index int) uint32 {
	return (uint32(key[index]) << 24) ^ (uint32(key[index+1]) << 16) ^ (uint32(key[index+2]) << 8) ^ (uint32(key[index+3]))
}
...
func PUTU32(ciphertext []uint8, st uint32) {
	ciphertext[0] = uint8((st) >> 24)
 ...
 */
func AesSetupEnc(rk []uint32, key []uint8, keybits int) int {
	var i int = 0
	var temp uint32
...
func AesEncrypt(rk []uint32, nrounds int, plaintext []uint8, ciphertext []uint8) {
