...
func PDUSessionEstablishmentAccept(ue *context.UEContext, smContext *context.SmContext) (msg []byte, err error) {
	...
	nasPdu, err := buildSessionEstablishmentAccept(*ue, smContext)
	if err != nil {
		return nil, err
	}
	return buildDLNASTransport(*ue, nasPdu, 1)
}

func buildDLNASTransport(ue context.UEContext, nasPdu []byte, pduSessionId uint8) (msg []byte, err error) {
	...
	return tools.Encode(&ue, m)
}

func buildSessionEstablishmentAccept(ue context.UEContext, smContext *context.SmContext) (msg []byte, err error) {
	m := nas.NewMessage()
	...
	if err != nil {
		return nil, errors.New("[AMF] Error while encoding qoSRules: " + err.Error())
	}
	...
	if byteArray, err := hex.DecodeString(nssai.Sd); err != nil {
		return nil, errors.New("[AMF] error while decoding nssai sd: " + err.Error())
	} else {
		copy(sd[:], byteArray)
	}
	...
	if smContext.ProtocolConfigurationOptions.DNSIPv4Request ||
		...
		if smContext.ProtocolConfigurationOptions.DNSIPv4Request {
			...
			if err != nil {
				return nil, errors.New("[AMF][NAS] Error while adding DNS IPv4 Addr: " + err.Error())
			}
		}
		...
		if smContext.ProtocolConfigurationOptions.DNSIPv6Request {
			...
			if err != nil {
				return nil, errors.New("[AMF][NAS] Error while adding DNS IPv6 Addr: " + err.Error())
			}
		}
