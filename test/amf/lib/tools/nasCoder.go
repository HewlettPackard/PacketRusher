...
func Encode(ue *context.UEContext, msg *nas.Message) ([]byte, error) {
	...
	if ue == nil || !ue.SecurityContextAvailable {
		...
		case nas.SecurityHeaderTypeIntegrityProtected:
			log.Debug("[AMF][NAS] Encoding Security header type: Integrity Protected")
		case nas.SecurityHeaderTypeIntegrityProtectedAndCiphered:
			log.Debug("[AMF][NAS] Encoding Security header type: Integrity Protected And Ciphered")
			needCiphering = true
		case nas.SecurityHeaderTypeIntegrityProtectedWithNew5gNasSecurityContext:
			log.Debug("[AMF][NAS] Encoding Security header type: Integrity Protected With New 5G Security Context")
...
func Decode(ue *context.UEContext, payload []byte, initialMessage bool) (msg *nas.Message, integrityProtected bool, err error) {
	if ue == nil {
		return nil, false, fmt.Errorf("[AMF][NAS] error while decoding: ue is nil")
	}
	if payload == nil {
		return nil, false, fmt.Errorf("[AMF][NAS] error while decoding: NAS payload is empty")
	}
	if len(payload) < 2 {
		return nil, false, fmt.Errorf("[AMF][NAS] error while decoding: NAS payload is too short")
	}
	...
	case nas.SecurityHeaderTypeIntegrityProtected:
		log.Info("[AMF][NAS] Message with integrity")

	case nas.SecurityHeaderTypeIntegrityProtectedAndCiphered:
		log.Info("[AMF][NAS] Message with integrity and ciphered")
		ciphered = true

	case nas.SecurityHeaderTypeIntegrityProtectedWithNew5gNasSecurityContext:
		log.Info("[AMF][NAS] Message with integrity and with NEW 5G NAS SECURITY CONTEXT")
		ulCountNew.Set(0, 0)

	case nas.SecurityHeaderTypeIntegrityProtectedAndCipheredWithNew5gNasSecurityContext:
		log.Info("[AMF][NAS] Message with integrity, ciphered and with NEW 5G NAS SECURITY CONTEXT")
		ciphered = true
		ulCountNew.Set(0, 0)
	default:
		return nil, false, fmt.Errorf("[AMF][NAS] wrong security header type: 0x%0x", msg.SecurityHeader.SecurityHeaderType)
	}

	if ciphered && !ue.SecurityContextAvailable {
		return nil, false, fmt.Errorf("[AMF][NAS] message is ciphered, but UE Security Context is not Available")
	}
	...
	if ue.SecurityContextAvailable {
		...
		if !reflect.DeepEqual(mac32, receivedMac32) {
			log.Info("[AMF][NAS] MAC verification failed(received: ", receivedMac32, ", expected: ", mac32, ")")
		} else {
			integrityProtected = true
		}
	} else {
		log.Info("[AMF][NAS] UE Security Context is not Available, so skip MAC verify")
	}
